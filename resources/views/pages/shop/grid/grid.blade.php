...
@section('title'){{ clean_for_html(@$title ?: _i("Tous les produits %s %s", [
  'text_gender' => text_for_seo__gender($data),
  'text_color' => empty($data['color']) ? '' : ucfirst($data['color']),
])) }}@stop
  ...
  <script type="application/ld+json">
    {!! json_encode([
      ...
      'itemListElement' => array_map(function($product) {
        ...
        $brand_slug = \Slugify::slugify($product['brand_original']);
        $brand_route = $brand_slug ? route('get.products.byBrand', ['brand' => $brand_slug]) : '#';
              ...
              <div class="main-heading-content">
                ...
                <div class="title-content text-center">
                  <div class="main-heading-title">
                    <h1>{{ get_prefix_for_paginated_page_title(': ') }}@yield('title')</h1>
                  </div>
      ...
      <div class="row shop-products">
                ...
                @section('aside_ref_text')
                  @if( !empty(@$data['ref_text']->text) )
                    <div class="aside-section aside-section-ref-text filter--section filter--section--no-shadow active">
                      ...
                      <div class="aside-section-content filter--section--body"><div>{!! clean_for_html($data['ref_text']->text, ['p','a','br', 'h3', 'b', 'strong', 'u', 'i']) !!}</div></div>
                    </div>
                  @endif
                @show
        ...
        <div
          ...
          <div id="shop-products-grid" class="shop-products-grid shop-products-grid--page--{{ $data['products_paginated']->resolveCurrentPage() }}">
            ...
            @php $animated_bg = get_random_gif() @endphp
            <div class="bloc-must-have product-thumb grid-columns">
              <div class="must-have-container lozad" data-background-image="{{ $animated_bg }}">
                <video autoplay muted loop playsinline class="lozad" preload="none" data-src="{{ $animated_bg }}"></video>
