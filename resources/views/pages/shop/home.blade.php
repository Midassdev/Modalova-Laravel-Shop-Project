...
@section('metas')
  ...
  @foreach(config('locales') as $locale => $locale_data)
    <link rel="alternate"
      hreflang="{{ str_replace('_', '-', $locale) }}"
      href="{{ $locale_data['url'] }}" />
  @endforeach
@stop
      ...
      <div class="main-heading-content">
        <div class="page-inspiration-intro">
          <div class="page-inspiration-intro-cta">
            ...
            <a class="btn btn-white hidden-xs" style="display: block; margin: 1.5em auto 0" href="{{ route('get.brands.index') }}" alt="{{ _i("Voir toutes nos marques | %s", ['site_name' => config('app.site_name'),]) }}">{{ _i("Nos Marques") }}</a>

            @if(config('features.enable_blog'))
              <a href="{{ url('/zine/') }}" class="btn btn-filled visible-xs" target="_blank">{{ _i("Magazine") }}</a>
            @endif
          </div>
        </div>
      </div>
  ...
  <div class="full-bg-grey">
    ...
    <div class="box-home box-home--intro container">
      ...
      <div class="box-home--intro--content">
        ...
        <p class="text-justify">{!! _i("home__our-mission") !!}<br><br></p>
      </div>
    </div>
