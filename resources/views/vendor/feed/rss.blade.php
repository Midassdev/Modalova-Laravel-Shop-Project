    ...
    <channel>
        ...
        @foreach($items as $item)
            <item>
                ...
                <enclosure url="{{ url(\App\Libraries\Cloudinary::get($item->image, '', $item->slug)) }}" type="audio/jpeg" length="78645" />

                <price>{{ $item->price . ' ' . $item->currency }}</price>
                <availability><![CDATA[in stock]]></availability>
                <image_link>{{ url(\App\Libraries\Cloudinary::get($item->image, '', $item->slug)) }}</image_link>
                ...
                <gender><![CDATA[{{ \App\Models\Gender::getNormalizedValue($item->gender) }}]]></gender>
