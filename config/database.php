...
if ($v = env('DATABASE_URL')) {
    ...
    if('sqlite' == @$db_url_from_heroku['scheme'])
        if(':memory:' != $db_url_from_heroku['host']) {
            $db_url_from_heroku['host'] = storage_path($db_url_from_heroku['host']);

            $f = fopen($db_url_from_heroku['host'], "a");
            if($f) fclose($f);
        }

    $db_url_from_heroku__follower = $db_url_from_heroku;

    $w = env('DATABASE_FOLLOWER_NAME');
    if ($w && $v = env($w)) {
