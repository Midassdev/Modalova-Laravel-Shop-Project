...
class AddPartialIndexesToWhiteList extends Migration
{
    ...
    public function down()
    {
        Schema::table('white_lists', function (Blueprint $table) {
            if ('postgres' == config('database.default')) {
                DB::unprepared('DROP INDEX IF EXISTS white_lists_brand_category_gender_unique_1');
                DB::unprepared('DROP INDEX IF EXISTS white_lists_brand_category_gender_unique_2');
            }
        });
    }
}
