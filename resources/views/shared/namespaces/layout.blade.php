...
<html lang="{{ str_replace('_', '-', config('app.locale')) }}">
  <head>
    ...
    <title>{{ get_prefix_for_paginated_page_title() }}@yield('title', $title){{ $suffix }}</title>
    ...
    <meta http-equiv="content-language" content="{{ str_replace('_', '-', config('app.locale')) }}">
    ...
    @section('javascripts')
      <script>
        var CONFIG = {
          ...
          ELASTICSEARCH_URL_PRODUCTS_INDEX: '{{ \App\Models\ProductFromElasticsearch::getIndexName() }}',
          APP_SEARCH: {
            endpoint: '{{ config('app_search.endpoint') }}',
            engine: '{{ config('app_search.engine') }}',
            key: '{{ config('app_search.public_key') }}',
          },
        };
      </script>
      <script>
