...
{
	function _i(string $string = null, $array = []) {
		if(empty($string))
			return '';
...
function get_prefix_for_paginated_page_title($separator = ' - ') {
	$page = LengthAwarePaginator::resolveCurrentPage('page', 1);

	if(1 == $page)
		return;

	return _i("Page %d{$separator}", [
		$page,
	]);
}
...
function get_robots_meta_tags($data = [])
{
	if(1 < $data['products_paginated']->resolveCurrentPage()) {
		if(@$data['category']->allow_indexing_of_paginated_pages)
			return [
				'content' => 'INDEX,FOLLOW',
				'reason'  => WhiteList::REASON_CATEGORY_ALLOWS_INDEXING,
			];
...
function create_cache_key($args)
{
		if(!is_array($args))
			$args = [$args];
...
function get_magic_route($params, $params_to_pass_on = [])
{
	$route = '';
	...
	return route('get.products'.$route, array_merge($params, $params_to_pass_on));
}
