...
class Modalova extends BaseSource
{
    ...
    private function get_campaign($id)
    {
        ...
        return [
            ...
            'category' => $category,
            'feeds' => $this->parse_note($dom->getElementById('note')->textContent, $category),
            'deep_link' => $link,
        ];
    }

    private function parse_note($string, $category)
    {
        $string = trim($string);
        ...
        foreach (preg_split("/\r\n|\n|\r/", $string) as $line) {
            ...
            if('shopify' == $category && false === strpos($link, '.json')) {
                $response = $this->client->get($link . '/collections.json');
                $data = json_decode($response->getBody()->getContents());

                foreach(@$data->collections ?: [] as $collection) {
                    $local_config = $config;

                    if(empty($local_config[Source::CONFIG_APPEND_CATEGORY]))
                        $local_config[Source::CONFIG_APPEND_CATEGORY] = '';

                    $local_config[Source::CONFIG_APPEND_CATEGORY] .= $collection->handle;

                    $feeds[] = [
                        'name' => $collection->title . ' ['.$collection->handle.']',
                        'link' => $link . '/collections/' . $collection->handle . '/products.json?limit=500',
                        'config' => $local_config,
                    ];
                }
            } else {
    ...
    {
        if (! $this->login()) {
            throw new \Exception('[-] Cannot update Sources from Modalova: could not login');
        }
