function load_elastic() {
    ...
    $pagination.pagination({
      ...
      onPageClick: function (page, event) {
        ...
        if(event) { // real click -- wont be defined when called from JS API
          ...
          new_href = location.pathname;

          if(page > 1) {
            // https://stackoverflow.com/a/41542008/1465029
            document.title = 'Page ' + page + ' - ' + CONFIG.DEFAULT_TITLE;

            if('URLSearchParams' in window) {
              var searchParams = new URLSearchParams(window.location.search);
              searchParams.set('page', page);
              new_href += '?' + searchParams.toString();
            } else {
              new_href += '?page=' + page;
            }
          } else {
        ...
        run_search(event ? new_href : null);
      }
    });
  }
  ...
  function run_search(new_href) {
    update_query_params();

    elasticsearch_client.search({
      index: CONFIG.ELASTICSEARCH_URL_PRODUCTS_INDEX,
      body: QUERY_PARAMS,
      requestCache: true,
    }).then(function (body) {
