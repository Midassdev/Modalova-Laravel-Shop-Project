...
class UpdateWhiteList extends Command
{
	...
	private function do_category($category, $brand, &$genders_detected, &$white_lists_created, $minimum_required_products = WhiteList::MINIMUM_REQUIRED_PRODUCTS)
	{
		...
		foreach ($genders_to_test as $gender) {
			...
			if(app()->runningInConsole()) {
				...
				echo str_pad("\tFound products (brand: {$brand->slug}, categ: {$category_slug}, gender: $gender, amount: $total): ", 100);
			}
		...
		if (isset($white_list) && app()->runningInConsole())
			echo "\n";
	...
	public function handle()
	{
		...
		foreach ($brands as $brand) {
			...
			if(app()->runningInConsole())
				echo "[=] Now cleaning old records of WhiteList\n";
			...
			if ($nb_old = $old_white_lists->count()) {
				...
				if(app()->runningInConsole())
					echo "[+] Removed '$nb_old' old records of WhiteList\n";
			}

			if(app()->runningInConsole()) {
				echo "[=] Detected " . count($genders_detected) . " genders: " . implode(array_keys($genders_detected)) . ". Current gender is {$brand->gender}\n";
			}
			...
			if ($new_gender != $brand->gender) {
				...
				if(app()->runningInConsole())
					echo "[+] (DISABLED) Brand ({$brand->slug}) is now ({$brand->gender}) only\n";
			}
		}
